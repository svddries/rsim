#ifndef ERA_MATH_MATRIX_H_
#define ERA_MATH_MATRIX_H_
...
public:
  ...
  Vec3T<T> row(int i) const {
      return Vec3T<T>(m[i*3], m[i*3+1], m[i*3+2]);
  }

  Vec3T<T> column(int i) const {
  ...
  void toQuaterion(QuaternionT<T>& q) const {
      T trace = xx + yy + zz;
  ...
  void fromQuaternion(const QuaternionT<T>& q) {
      T d = q.length2();
      T s = 2 / d;
